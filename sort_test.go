...
func testPartlySortedInts(t *testing.T, data []int, k int) {
	...
	if !IsSorted(data[:k]) {
		t.Fatalf("prefix is not sorted: %v", data[:k])
	}
	for i := 0; i < k; i++ {
		if data[i] != want[i] {
			t.Fatalf("prefix mismatch at %d: got %v want %v", i, data[:k], want[:k])
		}
	}
}
...
func TestPartlySort(t *testing.T) {
	cases := []struct {
		...
		{name: "zero", data: append([]int(nil), ints[:]...), k: 0},
		{name: "one", data: append([]int(nil), ints[:]...), k: 1},
		{name: "middle", data: append([]int(nil), ints[:]...), k: 5},
		{name: "all", data: append([]int(nil), ints[:]...), k: len(ints)},
...
func TestPartlySortFunc(t *testing.T) {
	data := []int{9, 7, 2, 8, 1, 6, 3, 5, 4}
	want := append([]int(nil), data...)
	sort.Ints(want)

	PartlySortFunc(data, 4, func(a, b int) bool { return a < b })
	if !IsSorted(data[:4]) {
		t.Fatalf("prefix is not sorted: %v", data[:4])
	}
	for i := 0; i < 4; i++ {
		if data[i] != want[i] {
			t.Fatalf("prefix mismatch at %d: got %v want %v", i, data[:4], want[:4])
		}
	}
}
